...
# Must use full path
# ANSI_SSH_PRIV_KEY="/Users/yabinmeng/.ssh/id_rsa_ymtest"
# ANSI_SSH_USER="automaton"
# ANSI_SSH_PRIV_KEY="/home/f667503/.ssh/id_rsa"
# ANSI_SSH_USER="mibapp"


DFT_LOG_HOMEDIR="nbs4j_exec_logs"

TC_EXEC_SCHEDULE_DEF_FILE="testcases/raw_definition/tc_exec_schedule"
TC_EXEC_SCHEDULE_DEF_DIR="testcases/raw_definition/"

usage() {
   echo
   echo "Usage: run_tc_by_schedule.sh [-h]"
   echo "                             [-logHomeDir <tc_exec_log_dir>"
   echo "                             [-ansiPrivKey <ansi_private_key>"
   echo "                             [-ansiSshUser <ansi_ssh_user>"
   echo "       -h   : Show usage info"
   echo "       [-tcExecScheduleFile]  : The filename of the testcase execution schedule file]"
...
while [[ "$#" -gt 0 ]]; do
   case $1 in
      -h) usage; exit 0 ;;
      -tcExecScheduleFile)  tcExecScheduleFile=$2; shift ;;
...
if [[ -z "${tcExecScheduleFile// }" ]]; then
    inExecScheduleFIle=${TC_EXEC_SCHEDULE_DEF_FILE}
else
    inExecScheduleFIle=${TC_EXEC_SCHEDULE_DEF_DIR}${tcExecScheduleFile}
fi
echo "Execute test cases by schedule (${inExecScheduleFIle})"
...
scheduleLogHomeDir="${logHomeDir}/by_schedule/${startTime2}"
...
outputMsg "Execute test cases by schedule (${inExecScheduleFIle})" 0 ${OUTPUT_TO_LOG}
...
while read LINE || [ -n "${LINE}" ]; do
    ...
    if [[ -d "testcases/raw_definition/${tcName}" ]]; then 
        ...
        if ! [[ ${tcPuaseTimeStr} =~ $re3 ]]; then
            echo "[ERROR] Invalid value testcase pause time string for line \"${LINE}\". It must be of format ${re3}"
            exit 40
        else
